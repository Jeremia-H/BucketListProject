...
function InspoPage() {
    ...
    return (
        ...
        <div className="w-full flex flex-col h-screen bg-customBg">
            <button className="absolute top-2 left-2  bg-customPurple p-2 rounded-xl hover:scale-105"
                    onClick={logOut}>
                <CgLogOut className="size-4 lg:size-5 text-white"></CgLogOut>
            </button>
            <div className="flex-1 bg-customBg pb-10 pt-10 w-full flex flex-col items-center justify-center pb-[9rem]">
                <h2 className="font-kalam font-bold text-xl text-center mb-4 sm:text-2xl md:text-4xl lg:text-5xl xl:text-6xl">Inspiration</h2>
                <main className="bg-customBg flex flex-wrap justify-center gap-1 w-full md:w-[50rem] xl:w-[60rem]">
                    {images.map((image, i) => (
                        <div key={i} className="relative size-[10rem] sm:size-[13rem] md:size-[15rem] lg:size-[16rem] xl:size-[17rem]">
                            <img
                                src={image.img}
                                className="w-full h-full object-cover rounded-lg"
                                alt={`Image ${i}`}
                            />
                            <div className="flex absolute inset-0 bg-black opacity-[0%] hover:opacity-[60%] transition-opacity duration-500 rounded-lg">
                                <p className="my-auto text-center w-full text-white">📍{image.city}, {image.country}</p>
                            </div>
                        </div>
                    ))}
                </main>
            ...
            <div
                ...
                <button className="">
                    <MdPhotoCamera className="size-7 text-customPurple sm:size-8"></MdPhotoCamera>
                </button>
                            ...
                            <div>
                                ...
                                <Formik
                                    ...
                                    onSubmit={async (values) => {
                                        const res = await onSubmitCreateList(values);
                                        ...
                                        */
                                        if (res instanceof Error) {
                                            toast.error(`Ups something went wrong: ${res.message}`, {
                                                position: "top-right",
